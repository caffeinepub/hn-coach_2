...
async function createAuthClientFromOptions(
...
export function InternetIdentityProvider({
  ...
  // Stable ref for createOptions - avoids re-running the init effect
  const createOptionsSnapshot = useRef(createOptions);
  ...
  const login = useCallback(() => {
    ...
    const options: AuthClientLoginOptions = {
      ...
      maxTimeToLive: ONE_HOUR_IN_NANOSECONDS * BigInt(24 * 365), // 1 year
    };
  ...
  // Initialize ONCE on mount. We read from a ref to avoid the dependency array
  // causing this effect to re-run (which would cause infinite loops).
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
        setStatus("initializing");
        const existingClient = await createAuthClientFromOptions(
          createOptionsSnapshot.current,
        );
        if (cancelled) return;
