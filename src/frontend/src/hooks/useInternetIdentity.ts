...
export function InternetIdentityProvider({
  ...
  // Use a ref to hold the authClient so it never triggers re-renders or effect re-runs
  ...
  const login = useCallback(() => {
    ...
    // Do NOT block returning users — just open Internet Identity popup
  ...
  const clear = useCallback(() => {
    ...
    void client
      ...
      .then(() => {
        ...
        setStatus("idle");
        setError(undefined);
        // Re-initialize after logout so a fresh client is available
  ...
  // Initialize ONCE on mount only — no dep array that re-triggers
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentionally run once only
  useEffect(() => {
